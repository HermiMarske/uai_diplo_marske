...
GO
/****** Object:  StoredProcedure [dbo].[AltaUsuario]    Script Date: 10/04/2019 23:13:30 ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	SELECT TOP 1 @existeUsuario = ID_Usuario FROM Usuarios WHERE usuario = @usuario;

	IF (NOT(@idPersona = 0) AND (@idUsuario = 0) AND (@existeUsuario = 0))
	BEGIN
	...
	ELSE IF (@idUsuario = 0 AND @existeUsuario = 0)
	BEGIN
	...
	END;
	ELSE IF (NOT(@existeUsuario = 0))
		SELECT TOP 1 'USER_EXISTS' as 'error';
	ELSE
