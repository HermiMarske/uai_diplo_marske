...
{
    public partial class modificacionPiloto : Form
    {
        public modificacionPiloto()
        {
            InitializeComponent();
        }
        ...
        private void modificacionPiloto_Load(object sender, EventArgs e)
        {
            piloto = ControladorABMPiloto.getPiloto((int)formInicio.idPilotoModif);
            ...
            txtNombre.Text = piloto.GetPersona().GetNombre();
            txtApellido.Text = piloto.GetPersona().GetApellido();
            txtDni.Text = piloto.GetPersona().GetDni();
            comboSexo.Text = piloto.GetPersona().GetSexo();
            pickerFechaNacimiento.Value = piloto.GetPersona().GetFechaNacimiento();

            txtLicencia.Text = piloto.GetLicencia();
            pickerEmision.Value = piloto.GetFechaExpedicionPsicofisico();
            pickerVencimiento.Value = piloto.GetFechaVencimientoPsicofisico();
        ...
        private void btnModificarPiloto_Click(object sender, EventArgs e)
        {
            ...
            Piloto p = piloto;
            Persona pers = p.GetPersona();


            pers.SetDni(txtDni.Text);
            pers.SetNombre(txtNombre.Text);
            pers.SetApellido(txtApellido.Text);
            pers.SetSexo(comboSexo.SelectedItem.ToString());
            pers.SetFechaNacimiento(pickerFechaNacimiento.Value);

            pers.SetDomicilios(domicilios);
            pers.SetMails(mails);
            pers.SetTelefonos(telefonos);

            p.SetPersona(pers);
            p.SetFechaExpedicionPsicofisico(pickerEmision.Value);
            p.SetFechaVencimientoPsicofisico(pickerVencimiento.Value);
            p.SetLicencia(txtLicencia.Text);

            messageBox.Show(ControladorABMPiloto.modificarPiloto(p));
        }
    }
}
